 ...
 *
 * Premium ocean-themed design:
 * - Gradient header strip
 * - Pixel crab side-by-side with brand text
...
// ═══════════════════════════════════════════════════════════════
// PIXEL CRAB — Ocean palette, matching the HTML design mockup
// 10 cols × 9 rows, compact and sharp
// ═══════════════════════════════════════════════════════════════

function getPixelCrab(): string[] {
  const D = chalk.hex('#0e7490');  // dark teal
  const M = chalk.hex('#0891b2');  // mid cyan
  const B = chalk.hex('#38bdf8');  // bright blue highlight
  const W = chalk.hex('#e0f7fa');  // white-ish (eyes)
  const T = chalk.hex('#2dd4bf');  // teal (claw tips)

  // Each row is exactly 10 visible chars wide
  return [
    `${D('█')}${' '}${' '}${' '}${' '}${' '}${' '}${' '}${' '}${D('█')}`,
    `${D('█')}${D('█')}${' '}${' '}${' '}${' '}${' '}${' '}${D('█')}${D('█')}`,
    `${' '}${D('█')}${M('▓')}${B('▒')}${M('▓')}${M('▓')}${B('▒')}${M('▓')}${D('█')}${' '}`,
...
// ═══════════════════════════════════════════════════════════════
// LOGO BLOCK — Pixel crab side-by-side with brand text
// ═══════════════════════════════════════════════════════════════
...
function renderLogoBlock(version: string): string {
  ...
  // Text lines aligned to vertical center of crab (9 rows)
  const textLines: string[] = [
    '',                // row 0 — claws
    supertitle,        // row 1 — ACR LABS
    title,             // row 2 — ShipMobile
    meta,              // row 3 — version + link
    '',
    '',
    '',
    '',
  ...
  const CRAB_COL_WIDTH = 12; // fixed column width for crab
  const result: string[] = [];

  for (let i = 0; i < crabLines.length; i++) {
    const crab = crabLines[i]!;
    const text = textLines[i] || '';
    const crabVisible = stripAnsi(crab).length;
    const pad = ' '.repeat(Math.max(0, CRAB_COL_WIDTH - crabVisible));
    result.push(`  ${crab}${pad}${text}`);
  }
